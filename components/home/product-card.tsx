...
import {
  ...
  useLoginPrompt,
} from "@/components/auth/login-prompt-provider";
...
export default function ProductCard({
  ...
  }, [productId, setWishlisted, startTransition]);

  useEffect(() => {
    const onAuthenticatedAction = (event: Event) => {
      const action = (event as CustomEvent<PendingAuthAction>).detail;
      if (isPendingAuthAction(action, { type: "wishlist", productId }))
        updateWishlist();
    };
    window.addEventListener(AUTH_ACTION_EVENT, onAuthenticatedAction);
    return () =>
      window.removeEventListener(AUTH_ACTION_EVENT, onAuthenticatedAction);
  }, [productId, updateWishlist]);
  ...
  const handleWishlist = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    event.stopPropagation();
    if (!isAuthenticated) {
      requestLogin({ type: "wishlist", productId });
      return;
    }
  ...
  return (
    ...
    >
      <div className="relative mb-3">
        <div className="absolute left-0 top-0 z-10 flex gap-2 opacity-100 transition-opacity sm:opacity-0 sm:group-hover:opacity-100">
          <Tooltip label="Add to cart">
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded-full bg-white text-gray-700 shadow-md transition-colors hover:bg-primary hover:text-white disabled:opacity-50"
            >
              <ShoppingCart className="h-4 w-4" />
            </button>
          </Tooltip>
          <Tooltip label={isWishlisted(productId) ? "Remove from wishlist" : "Add to wishlist"}>
            <button
              type="button"
              onClick={handleWishlist}
              disabled={isPending}
              aria-label={`${isWishlisted(productId) ? "Remove" : "Add"} ${name} ${isWishlisted(productId) ? "from" : "to"} wishlist`}
              className={`flex h-8 w-8 items-center justify-center rounded-full bg-white shadow-md transition-colors hover:bg-rose-200 disabled:opacity-50 ${isWishlisted(productId) ? "text-rose-500" : "text-gray-700"}`}
            >
        ...
        {!!discount && (
          <span className="absolute right-0 top-0 z-10 rounded-md bg-primary px-2 py-1 text-xs font-semibold text-white">
            {discount}% OFF
          </span>
        )}
